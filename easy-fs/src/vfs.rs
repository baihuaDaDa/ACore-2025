...
use crate::layout::{DirEntry, DiskInode, DiskInodeType, DIRENTRY_SIZE};
...
impl Inode {
    ...
    pub fn ls(&self) -> Vec<String> {
        let _fs = self.fs.lock(); // no warning from compiler
        self.read_disk_inode(|disk_inode| {
            let file_count = (disk_inode.size as usize) / DIRENTRY_SIZE;
            let mut dirent = DirEntry::empty();
            let mut v = Vec::new();
    ...
    pub fn clear(&self) {
        ...
        self.modify_disk_inode(|disk_inode| {
            ...
            for data_blocks in data_blocks_dealloc.into_iter() {
                fs.dealloc_data(data_blocks);
            }
        });
    }
